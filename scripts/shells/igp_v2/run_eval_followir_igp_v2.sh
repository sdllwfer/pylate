...
CUSTOM_OUTPUT_PATH="/home/luwa/Documents/pylate/evaluation_data/colbert_igp/col_two_stage_short_then_long_v2"
# 自定义输出路径 (可选)
NOTE='V2端到端模型评测'
# ==============================
...
if [ -n "${CUSTOM_OUTPUT_PATH}" ]; then
    ...
    OUTPUT_DIR="${OUTPUT_BASE_DIR}/${MODEL_TIMESTAMP}_eval${EVAL_TIMESTAMP}_v2"
    echo "📂 使用时间戳输出路径: ${OUTPUT_DIR}"
fi

echo "============================================================"
echo "📊 IGP V2 流式评测流程（增加参数量版本）"
