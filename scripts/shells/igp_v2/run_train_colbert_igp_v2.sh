...
#   2. 运行脚本: bash scripts/shells/run_train_colbert_igp_v2.sh
#
...
CUSTOM_OUTPUT_PATH="/home/luwa/Documents/pylate/output/colbert_igp_train/col_igp_v2_large"
NOTE="ColBERT-IGP-V2-大参数量版本"
...
MAX_RATIO=0.2
# Adapter 瓶颈维度 (V2: 512, 原V1: 128)
BOTTLENECK_DIM=512
# Probe 编码器层数 (V2: 6, 原V1: 2)
PROBE_NUM_LAYERS=6
# 辅助损失权重
AUX_LOSS_WEIGHT=0
...
if [ -n "${CUSTOM_OUTPUT_PATH}" ]; then
    ...
    OUTPUT_DIR="${OUTPUT_BASE_DIR}/${TIMESTAMP}_v2"
...
echo "============================================================"
echo "� ColBERT-IGP V2 两阶段训练（增加参数量版本）"
echo "============================================================"
echo "模型: ${MODEL_NAME}"
...
echo "Adapter瓶颈维度: ${BOTTLENECK_DIM} (V2: 512, V1: 128)"
echo "Probe层数: ${PROBE_NUM_LAYERS} (V2: 6, V1: 2)"
...
# 运行训练 (使用 V2 版本的 Python 脚本)
python -u scripts/training/train_colbert_igp_v2.py \
