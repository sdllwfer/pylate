...
# 调用 compute_loss
print("\n📍 调用 compute_loss...")
loss_outputs = trainer.compute_loss(features, labels, return_outputs=True)

if isinstance(loss_outputs, tuple):
    loss = loss_outputs[0]
    outputs = loss_outputs[1] if len(loss_outputs) > 1 else None
else:
    loss = loss_outputs
    outputs = None
